...
const RPC = [
  ...
  ['agentPreset.update', 'Agent Presets',
    { agentPreset: 'my-preset', name: 'My Preset', description: 'One sentence on what this preset is for.' },
    { name: 'My Preset', description: 'One sentence on what this preset is for.' },
    'Sets a locally authored preset\'s display `name` and/or `description` in its `preset.yml`, preserving `order` and `workspacePath` (composition stays copy-only). A present value sets it, an empty string clears it, an omitted field keeps the current one; the response reports the effective values. Shipped presets refused (agent-preset-read-only). Privileged authoring: loopback-pinned, OR callable by an authenticated client when the deployment lists it in `auth.unpinned`.'],
