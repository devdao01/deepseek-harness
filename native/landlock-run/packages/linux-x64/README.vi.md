...
Launcher Landlock `bin/landlock-run` dựng sẵn cho linux-x64: một file nhị phân musl tĩnh (không dùng cross toolchain) được biên dịch gốc từ mã nguồn C đi kèm gói [`@deepseek-ai/node-addon-landlock-run`](https://www.npmjs.com/package/@deepseek-ai/node-addon-landlock-run). Trường `os`/`cpu` của npm chọn gói này lúc cài đặt; gói entry định vị nó tới đường dẫn file. Gói này không chứa JavaScript, và không bao giờ được import.
...
Gói cùng cấp: `@deepseek-ai/node-addon-landlock-run-linux-arm64`.
