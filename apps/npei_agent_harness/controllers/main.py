...
from ..models.harness_client import HarnessWireError, _wire_for
...
class MtilAgentController(http.Controller):
    ...
    def _wire(self):
        """Return the authenticated harness wire, or ``None`` when unconfigured."""
        try:
            base_url, token = request.env['npei.agent.harness.client'].sudo()._get_connection()
        except UserError:
            return None
        return _wire_for(base_url, token)
    ...
    def rpc_proxy(self, method, **kwargs):
        ...
        try:
            upstream = wire.raw_post('api/%s' % method, raw)
        except (HarnessWireError, requests.RequestException) as exc:
