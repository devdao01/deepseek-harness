...
| Shipped preset display copy | `packages/preset/agent-presets/presets/{standard,ptc,minimal,cordis}/preset.yml` | All four shipped presets carry English `name`/`description` verbatim from their `preset<Id>Name`/`preset<Id>Description` keys (`packages/client/ui-agent-preset/src/client/locales.ts`) instead of upstream's Chinese. Raw roster metadata is what every non-localized consumer renders — the Odoo mirror (`npei.agent.preset`) and Host surfaces — while `presetDisplayText` still routes client UI through those same locale keys, so only the fallback changes and both paths now read alike. **On merge, keep the English copy**, and re-copy it when upstream edits either locale value: resolving in upstream's favour silently restores Chinese in the Odoo backend, and no test reads the shipped files, so nothing fails to warn you | rc.2 merge |
| MTIL UI flags in stock client packages | `packages/client/ui-workspace/src/client/{tree,rows/*}.ts(x)`, `ui-conversation/locales.ts`, `ui-chat/apply.ts`, `ui-sidebar/*`, `ui-model-selection`, `ui-agent-preset/settings-store.ts` | `__MTIL_UI__` gates, all set in `apps/frontend/src/main.ts` and absent on the harness-served original: titled blank sessions listed, hero headline override, download-not-open for files, sidebar header-actions slot, custom model picker seat, deactivated presets withheld from pickers, and the lifecycle withholdings below | `b643e9ad`, `aa2293a9`, `2537206a`, `84d60d72`, `178ba61b` |
| Odoo owns Session and Workspace lifecycle | `packages/client/ui-workspace/src/client/rows/Rows.tsx`, `packages/client/ui-conversation/src/client/skeleton/ConversationRoot.tsx` | Odoo creates every session, holds the ACL, and names both, so the SPA offers no lifecycle verb: `hideNewSession` withholds the per-row new-session button, the hero's Workspace chip row (its picker retargets a blank Session's Workspace, which mints one where that Workspace has no blank Session yet), and — with nothing selected — the composer, leaving a hero that only greets; `hideWorkspaceActions` withholds the Workspace row's rename/delete, `hideSessionActions` the Session row's rename/fork/archive. The composer survives on a SELECTED blank Session: typing its first prompt is what Odoo created it for, and `skeleton.client.spec.tsx` → `MTIL blank Session: the composer survives so the first prompt can be typed` is what keeps a future gate from taking it away. Each menu's anchor disappears with the last of its items. **On merge, keep every gate and re-apply it to whatever replaces these rows** — an upstream refactor that drops one silently returns a destructive verb (delete Workspace, archive Session) to users who must not have it, and the only signal is `ui-workspace/tests/rows.client.spec.tsx` → `withholds both row menus under the MTIL lifecycle flags`. A rule in the SPA's own `custom.css` was the zero-conflict alternative and lost: these rows carry no `data-slot`, so the only selector is a per-build CSS-module hash whose rename would restore the verbs with nothing failing | this change |
