...
| Access gate on upstream's file surfaces | `packages/api/workspace-files/src/index.ts`, `packages/api/session-controller/src/index.ts` | `workspaceFiles` takes the same `ticketSecret` and resolves a Session the caller may not read to nothing, so all seven Remotes refuse it as unknown. `/api/file` names no Session and cannot be gated, so a ticketed deployment does not mount it at all — the overlay MUST set `ticketSecret` on **both** `session-controller` and `workspace-files` | 0.1.5 rebase |
