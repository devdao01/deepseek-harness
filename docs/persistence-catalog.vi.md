<!-- Tệp nguồn tiếng Anh do scripts/gen-persistence-catalog.ts sinh ra; tệp tiếng Việt này là bản đối chiếu đã qua đánh giá, được duy trì thông qua ghép cặp song ngữ.
     Khi cập nhật, hãy chạy `pnpm run gen-persistence-catalog` để cập nhật bản tiếng Anh trước, rồi cập nhật tệp này và chạy `pnpm run verify-translation-pairing --write docs/persistence-catalog.md` để ghi lại ghép cặp. -->

# Danh mục sự kiện persistence của phiên

[English](persistence-catalog.md) | Tiếng Việt

Mọi kiểu sự kiện có thể xuất hiện trong nhật ký sự kiện bền vững của một phiên: phong bì `SessionEvent` được persist đầy đủ, cùng từng thành viên của `SessionEventMap` có thể mở rộng bằng declaration merging, bao gồm bộ từ vựng thuộc `@deepseek-ai/dsh-session` và mọi declaration merge của từng plugin trong repo này vào `@deepseek-ai/dsh-session/types`, kèm JSDoc nguồn, khai báo payload đầy đủ, nhãn surface và vị trí khai báo. Tài liệu này bổ sung cho [session.md](subsystems/session.md) (thứ tự surface và phép chiếu `deriveMessages()`), [persistence.md](subsystems/persistence.md) (cách làm cho nhật ký trở nên bền vững) và vùng được sinh ra trong [session.md](subsystems/session.md#cordis-surface) (đấu nối bus thời gian thực; sự kiện nhật ký **không phải** là sự kiện cordis, nó đến với listener qua duy nhất một lần emit `session/event`).

Tệp nguồn tiếng Anh được sinh ra từ mã nguồn (`scripts/gen-persistence-catalog.ts`) và được `pnpm run verify-persistence-catalog` (một phần của `doc-sync`, cổng đồng bộ tài liệu) kiểm chứng tính mới; tệp tiếng Việt này được duy trì như bản đối chiếu đã qua đánh giá thông qua ghép cặp song ngữ. Các khối khai báo giữ nguyên khai báo trong mã nguồn và JSDoc của các thuộc tính lồng nhau, chỉ loại bỏ phần thụt lề do interface／module chứa chúng gây ra, và dùng fence `ts persistence-catalog` (doc-typecheck bỏ qua các fence này vì khai báo tham chiếu tới các kiểu trong module sở hữu chúng). Tên kiểu trong payload sẽ liên kết tới trang tài liệu hóa kiểu đó. Xem [Agent Note persistence-log-catalog](../.agents/notes/archived/process/2026-07-04-persistence-log-catalog.md).

Các khai báo phong bì dưới đây kết hợp `type` của từng sự kiện, `seq` tăng đơn điệu, `time` tính bằng mili giây epoch, `data`, cờ bỏ qua kiểu chưa biết tùy chọn `ignorable`, cùng các trường có điều kiện `surfaceOp`／`sourceEventSeqs`. **surface** đánh dấu một thành viên `SurfaceEventType`: nó sinh ra một thông điệp LLM (mô hình ngôn ngữ lớn) và khai báo cách sự kiện đó gia nhập danh sách surface. **log-only** đánh dấu tất cả những sự kiện còn lại: đây là bản ghi bền vững, có thể phát lại, nhưng không đóng góp vào lịch sử phái sinh. Mọi payload đều có thể tuần tự hóa JSON (được cưỡng chế tại `Session.append`), và toàn bộ định dạng được ghim ở `SESSION_FORMAT_VERSION = 0`: đây là định dạng tiền phát hành, không hàm ý bất kỳ cam kết tương thích nào (xem [lập trường về phiên bản](subsystems/persistence.md)). Phạm vi chỉ giới hạn trong các package của repo này; plugin phía hạ nguồn có thể merge thêm các kiểu sự kiện khác, và theo thiết kế những kiểu đó nằm ngoài danh mục này.
...
Nguồn: [`packages/core/session/src/types.ts:336`](../packages/core/session/src/types.ts) · [`packages/core/session/src/types.ts:343`](../packages/core/session/src/types.ts) · [`packages/core/session/src/types.ts:372`](../packages/core/session/src/types.ts) · [`packages/core/session/src/types.ts:404`](../packages/core/session/src/types.ts)
...
Nguồn: [`packages/core/agent/src/types.ts:19`](../packages/core/agent/src/types.ts)
...
Nguồn: [`packages/preset/agent-presets/src/session.ts:26`](../packages/preset/agent-presets/src/session.ts)
...
Kiểu: [CallId](subsystems/core.md)

Nguồn: [`packages/interaction/user-approval/src/index.ts:44`](../packages/interaction/user-approval/src/index.ts)
...
Nguồn: [`packages/interaction/user-approval/src/index.ts:55`](../packages/interaction/user-approval/src/index.ts)
...
Nguồn: [`packages/interaction/user-approval/src/index.ts:67`](../packages/interaction/user-approval/src/index.ts)
...
Kiểu: [StreamChunk](subsystems/llm-streaming.md)

Nguồn: [`packages/core/session/src/types.ts:266`](../packages/core/session/src/types.ts)
...
Kiểu: [TokenUsage](subsystems/llm-streaming.md)

Nguồn: [`packages/core/session/src/types.ts:273`](../packages/core/session/src/types.ts)
...
Nguồn: [`packages/interaction/commands/src/types.ts:95`](../packages/interaction/commands/src/types.ts)
...
Nguồn: [`packages/interaction/commands/src/types.ts:88`](../packages/interaction/commands/src/types.ts)
...
Nguồn: [`packages/compaction/compaction/src/types.ts:71`](../packages/compaction/compaction/src/types.ts)
...
Nguồn: [`packages/compaction/compaction/src/types.ts:81`](../packages/compaction/compaction/src/types.ts)
...
Nguồn: [`packages/compaction/compaction/src/types.ts:23`](../packages/compaction/compaction/src/types.ts)
...
Kiểu: [ContentBlock](subsystems/core.md) · [TokenUsage](subsystems/llm-streaming.md)

Nguồn: [`packages/compaction/compaction/src/types.ts:33`](../packages/compaction/compaction/src/types.ts)
...
Nguồn: [`packages/feedback/command-feedback/src/index.ts:62`](../packages/feedback/command-feedback/src/index.ts)
...
Nguồn: [`packages/goal/goal/src/domain.ts:66`](../packages/goal/goal/src/domain.ts)
...
Nguồn: [`packages/hooks/hook-protocol/src/types.ts:19`](../packages/hooks/hook-protocol/src/types.ts)
...
Nguồn: [`packages/hooks/hook-protocol/src/types.ts:31`](../packages/hooks/hook-protocol/src/types.ts)
...
Nguồn: [`packages/llm/llm-retry/src/types.ts:9`](../packages/llm/llm-retry/src/types.ts)
...
Nguồn: [`packages/llm/llm-retry/src/types.ts:11`](../packages/llm/llm-retry/src/types.ts)
...
Nguồn: [`packages/interaction/permission-presets/src/index.ts:50`](../packages/interaction/permission-presets/src/index.ts)
...
Nguồn: [`packages/plan/plan-mode/src/index.ts:53`](../packages/plan/plan-mode/src/index.ts)
...
Nguồn: [`packages/core/session/src/types.ts:309`](../packages/core/session/src/types.ts)
...
Nguồn: [`packages/core/session/src/types.ts:304`](../packages/core/session/src/types.ts)
...
Nguồn: [`packages/sandbox/sandbox-policy/src/session-mode.ts:33`](../packages/sandbox/sandbox-policy/src/session-mode.ts)
...
Kiểu: [ScheduleChange](subsystems/schedule.md)

Nguồn: [`packages/schedule/schedule/src/types.ts:219`](../packages/schedule/schedule/src/types.ts)
...
Nguồn: [`packages/core/session/src/types.ts:332`](../packages/core/session/src/types.ts)
...
Kiểu: [SessionTitleEventData](subsystems/session-title.md)

Nguồn: [`packages/session/session-title/src/index.ts:100`](../packages/session/session-title/src/index.ts)
...
Kiểu: [SessionTitleLlmRequestEventData](subsystems/session-title.md)

Nguồn: [`packages/session/session-title-llm/src/index.ts:43`](../packages/session/session-title-llm/src/index.ts)
...
Nguồn: [`packages/core/session/src/types.ts:256`](../packages/core/session/src/types.ts)
...
Nguồn: [`packages/core/session/src/types.ts:254`](../packages/core/session/src/types.ts)
...
Nguồn: [`packages/subagent/subagent/src/descriptor.ts:37`](../packages/subagent/subagent/src/descriptor.ts)
...
Kiểu: [TodoItem](subsystems/session.md)

Nguồn: [`packages/core/session/src/types.ts:299`](../packages/core/session/src/types.ts)
...
Nguồn: [`packages/core/session/src/types.ts:279`](../packages/core/session/src/types.ts)
...
Nguồn: [`packages/core/tools/src/types.ts:56`](../packages/core/tools/src/types.ts)
...
Nguồn: [`packages/core/tools/src/types.ts:40`](../packages/core/tools/src/types.ts)
...
Nguồn: [`packages/core/session/src/types.ts:291`](../packages/core/session/src/types.ts)
...
Nguồn: [`packages/workflow/tool-workflow/src/types.ts:57`](../packages/workflow/tool-workflow/src/types.ts)
...
Nguồn: [`packages/workflow/tool-workflow/src/types.ts:52`](../packages/workflow/tool-workflow/src/types.ts)
...
Nguồn: [`packages/workflow/tool-workflow/src/types.ts:62`](../packages/workflow/tool-workflow/src/types.ts)
...
Nguồn: [`packages/workflow/tool-workflow/src/types.ts:47`](../packages/workflow/tool-workflow/src/types.ts)
...
Kiểu: [TurnEndReason](subsystems/session.md)

Nguồn: [`packages/core/session/src/types.ts:252`](../packages/core/session/src/types.ts)
...
Nguồn: [`packages/core/session/src/types.ts:243`](../packages/core/session/src/types.ts)
...
Nguồn: [`packages/core/session/src/types.ts:264`](../packages/core/session/src/types.ts)
...
Nguồn: [`packages/web/web-search-deepseek/src/provider.ts:83`](../packages/web/web-search-deepseek/src/provider.ts)
