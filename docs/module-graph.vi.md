<!-- Tệp nguồn tiếng Anh do scripts/gen-module-graph.ts sinh ra; tệp tiếng Việt này là bản đối chiếu đã được rà soát, duy trì qua ghép cặp song ngữ.
     Khi cập nhật, trước tiên chạy `pnpm run gen-module-graph` để cập nhật bản tiếng Anh, sau đó cập nhật tệp này rồi chạy `pnpm run verify-translation-pairing --write docs/module-graph.md` để ghi lại cặp ghép. -->

# Sơ đồ phụ thuộc module

[English](module-graph.md) | Tiếng Việt

Quan hệ phụ thuộc giữa các package harness `@deepseek-ai/dsh-*`. Sơ đồ này được sinh ra dựa trên `peerDependencies` của từng package (tín hiệu phụ thuộc runtime chuẩn), và được nhóm theo cấp `packages/<group>/<pkg>`. Cạnh `a --> b` biểu thị package `a` phụ thuộc vào package `b`. Tiền tố `@deepseek-ai/dsh-` trong tên đã được lược bỏ.
