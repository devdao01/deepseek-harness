<!-- Tệp nguồn tiếng Anh do scripts/gen-tool-catalog.ts sinh ra; tệp tiếng Việt này là bản đối chiếu đã được rà soát, duy trì qua ghép cặp song ngữ.
     Khi cập nhật, trước tiên chạy `pnpm run gen-tool-catalog` để cập nhật bản tiếng Anh, sau đó cập nhật tệp này rồi chạy `pnpm run verify-translation-pairing --write docs/tool-catalog.md` để ghi lại cặp ghép. -->

# Danh mục Schema công cụ

[English](tool-catalog.md) | Tiếng Việt

Tất cả công cụ hướng đến model mà các plugin đã phát hành cung cấp cho `ctx.tools`: `name`, `description` và JSON Schema `parameters` mà model nhận được qua quá trình lắp ráp system prompt. Danh mục này bổ sung cho [trang subsystem](subsystems/core.md) (các kiểu và vùng đấu nối `cordis-surface` được sinh ra trên từng trang); trang này liệt kê các *công cụ* được cung cấp cho agent.

Tệp nguồn tiếng Anh được hệ thống **sinh ra** và được kiểm tra độ mới bằng `pnpm run verify-tool-catalog` (một phần của `doc-sync` (cổng đồng bộ tài liệu)); tệp tiếng Việt này được duy trì như bản đối chiếu đã rà soát thông qua ghép cặp song ngữ. Khác với danh mục Cordis (chỉ xử lý AST của mã nguồn), trình sinh bản tiếng Anh **khởi động** từng plugin công cụ trong ngữ cảnh thật rồi đọc `ctx.tools.schemas()`, vì schema công cụ không thể xác định trọn vẹn bằng phân tích tĩnh, ví dụ enum được mở rộng lúc chạy, mô tả được ghép chuỗi, tên do cấu hình quyết định và các công cụ MCP dùng JSON Schema thô. Bộ bảo vệ tính đầy đủ khớp glob `packages/*/tool-*`; nếu manifest khởi động của trình sinh bỏ sót bất kỳ package nào thì kiểm tra sẽ thất bại, nhờ vậy công cụ mới không thể thiếu tài liệu mà không ai hay biết. Xem [Agent Note về danh mục schema công cụ](../.agents/notes/implemented/process/2026-07-02-tool-schema-catalog.md).

Phạm vi: các công cụ sản phẩm đã phát hành nằm dưới `packages/*/tool-*`, mỗi công cụ đều được khởi động bằng cấu hình **mặc định** của nó; nhưng nếu một trường Config là **bắt buộc** và không có giá trị mặc định thì trình sinh buộc phải chọn, và ghi chú của package tương ứng sẽ nêu rõ trang này trình bày nhánh nào. **Tên** công cụ được đăng ký có thể là cấu hình lúc nạp, ví dụ `toolName` của `tool-subagent`, nên một triển khai có thể cung cấp package đó dưới tên khác hoặc dưới thêm một tên nữa; nếu có bí danh đi kèm sản phẩm, ghi chú của package tương ứng sẽ ghi lại. Các công cụ minh họa trong `examples/` (ví dụ `echo`) nằm ngoài phạm vi, thống nhất với việc danh mục Cordis chỉ bao gồm các package.

## Ánh xạ package công cụ

Bảng dưới đây ánh xạ tên công cụ mà model nhìn thấy với package plugin và service seam đứng sau nó. Các mục theo package ở phần sau đưa ra JSON Schema chính xác.

| Package công cụ | Tên model nhìn thấy | Phụ thuộc | Ghi/ảnh hưởng | Bí danh đi kèm sản phẩm | Ghi chú triển khai |
| --- | --- | --- | --- | --- | --- |
...
| `@deepseek-ai/dsh-schedule` | `schedule_create`、`schedule_delete`、`schedule_list` | `ctx.tools`, `ctx.sessions`, bền vững hóa Session, Agent gốc live được tạo trong tương lai | `tool/call`、`schedule/change create or delete`、`tool/result` | - | 仅在选择启用的 Schedule 插件加载后创建的 live 根 Agent scope 内注册。版本 1 接受 after_seconds、显式绝对 at 和有界固定速率 every_seconds，并披露 session-local 交付；管理读取与变更必须通过共享的 Session 持久化 barrier。 |
