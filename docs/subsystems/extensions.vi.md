...
[English](extensions.md) | Tiếng Việt

Hệ thống con extensions cho phép agent định nghĩa các package Cordis có đánh phiên bản, chạy cả hai nửa host và trình duyệt của chúng, và truy vấn metadata runtime được phép công khai trước khi viết code. Vòng đời package và hành vi sandbox được mô tả trong nhóm package [`packages/extensions`](../../packages/extensions/README.md).
