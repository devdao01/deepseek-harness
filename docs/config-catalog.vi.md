<!-- Tệp nguồn tiếng Anh do scripts/gen-config-catalog.ts sinh ra; tệp tiếng Việt này là bản đối chiếu đã được rà soát, duy trì qua ghép cặp song ngữ.
     Khi cập nhật, trước tiên chạy `pnpm run gen-config-catalog` để cập nhật bản tiếng Anh, sau đó cập nhật tệp này rồi chạy `pnpm run verify-translation-pairing --write docs/config-catalog.md` để ghi lại cặp ghép. -->

# Danh mục cấu hình plugin

[English](config-catalog.md) | Tiếng Việt

Mỗi khối `config:` đều có thể được thiết lập từ một entry trong `cordis.yml`: với mỗi package harness có thể tải, tài liệu này liệt kê nguyên trạng khai báo cấu hình mà hàm `apply` hoặc hàm khởi tạo service của package đó nhận vào (bao gồm cả JSDoc), kèm theo mọi kiểu được tham chiếu — kiểu nội bộ package được dán trực tiếp, kiểu khác thì cung cấp liên kết. Nội dung được dán là kiểu cấu hình đầy đủ mà plugin khai báo — các trường mà runtime schema cố tình loại trừ là seam chỉ dùng cho runtime (JSDoc của chính trường đó sẽ nêu rõ điều này) và không thể thiết lập qua `cordis.yml`. Đây là tài liệu tham chiếu lấy **triển khai (deployment)** làm trục — cách kết nối mà tác giả plugin dựa vào xem tại vùng `cordis-surface` được sinh ra trong từng [trang subsystem](subsystems/core.md), schema công cụ hướng tới model xem [danh mục công cụ](tool-catalog.md), còn [subsystems/](subsystems/core.md) ghi lại các kiểu mà những khai báo này tham chiếu tới.

Tệp nguồn tiếng Anh được sinh ra từ mã nguồn (`scripts/gen-config-catalog.ts`) và được xác thực độ mới thông qua `pnpm run verify-config-catalog` (một phần của `doc-sync`); tệp tiếng Việt này được duy trì như bản đối chiếu đã qua rà soát trong cặp song ngữ. Khối khai báo dùng fence `ts config-catalog` (doc-typecheck sẽ bỏ qua khối này, vì khai báo chỉ tham chiếu riêng lẻ các import không thể tự biên dịch độc lập). Bộ sinh tiếng Anh còn đối chiếu chéo schema schemastery của runtime với khai báo đã dán — mọi khóa đã được schema xác thực (kể cả khóa lồng nhau) đều phải tìm thấy trong kiểu cấu hình đã khai báo — do đó nội dung dán không thể che giấu trường mà loader chấp nhận.

Dòng `Requires:` liệt kê các service key mà plugin inject qua `inject`: cây `cordis.yml` của plugin đó cũng phải tải provider của các service này. Phạm vi giới hạn ở tầng harness (`packages/`); các plugin cordis vendored mà cây cấu hình có thể tải thêm (`hmr`, bộ ghi log console, v.v.) được cố định theo mã nguồn thượng nguồn (xem [vendoring policy](../vendor/README.md)) và không được liệt kê trong danh mục này.
...
Phụ thuộc: `Stream`（`@agentclientprotocol/sdk`）

Nguồn: [`packages/acp/acp/src/index.ts:71`](../packages/acp/acp/src/index.ts)
...
Phụ thuộc: [`agentCore`](../packages/examples/agent-spine-demo/src/index.ts) · [`JsonlCompression`](../packages/session/session-persistence-jsonl/src/index.ts) · [`ToolsConfig`](#deepseek-aidsh-tools)

Nguồn: [`packages/examples/acp-demo/src/index.ts:39`](../packages/examples/acp-demo/src/index.ts)
...
Nguồn: [`packages/core/agent-default-model/src/index.ts:41`](../packages/core/agent-default-model/src/index.ts)
...
Nguồn: [`packages/context/agent-instructions/src/config.ts:18`](../packages/context/agent-instructions/src/config.ts)

<a id="deepseek-aidsh-agent-loop"></a>

## `@deepseek-ai/dsh-agent-loop`

Yêu cầu: `agents` · `sessions` · `llm` · `tools` · `systemPrompt`
...
Phụ thuộc: [`AgentOptions`](subsystems/core.md) · [`SessionId`](subsystems/core.md)

Nguồn: [`packages/core/agent-loop/src/index.ts:255`](../packages/core/agent-loop/src/index.ts)
...
Nguồn: [`packages/preset/agent-presets/src/preset.ts:59`](../packages/preset/agent-presets/src/preset.ts)
...
Phụ thuộc: [`ToolPresentationMode`](subsystems/tools.md)

Nguồn: [`packages/core/agent-tool-presentation/src/index.ts:38`](../packages/core/agent-tool-presentation/src/index.ts)
...
Nguồn: [`packages/attachment/attachment-local/src/index.ts:24`](../packages/attachment/attachment-local/src/index.ts)
...
Nguồn: [`packages/shell/bash-local/src/index.ts:41`](../packages/shell/bash-local/src/index.ts)

<a id="deepseek-aidsh-bash-sandbox"></a>

## `@deepseek-ai/dsh-bash-sandbox`

Yêu cầu: `subprocess` · `sandbox` · `sandboxPolicy`
...
Phụ thuộc: [`LocalConfig`](#deepseek-aidsh-bash-local)

Nguồn: [`packages/shell/bash-sandbox/src/index.ts:35`](../packages/shell/bash-sandbox/src/index.ts)
...
Phụ thuộc: [`TicketAuthConfig`](../packages/identity/user-ticket/src/index.ts)

Nguồn: [`packages/client/connection/src/index.ts:61`](../packages/client/connection/src/index.ts)

<a id="deepseek-aidsh-client-hmr"></a>

## `@deepseek-ai/dsh-client-hmr`

Yêu cầu: `clientModuleHost` · `webServer`
...
Nguồn: [`packages/client/hmr/src/index.ts:31`](../packages/client/hmr/src/index.ts)
...
Nguồn: [`packages/code-runtime/code-runtime-worker-thread/src/index.ts:25`](../packages/code-runtime/code-runtime-worker-thread/src/index.ts)

<a id="deepseek-aidsh-compaction-basic"></a>

## `@deepseek-ai/dsh-compaction-basic`

Yêu cầu: `llm` · `tokenMeter` · `sessions`
...
Nguồn: [`packages/compaction/compaction-basic/src/types.ts:38`](../packages/compaction/compaction-basic/src/types.ts)
...
Nguồn: [`packages/compaction/compaction-tool-result-pruner/src/types.ts:4`](../packages/compaction/compaction-tool-result-pruner/src/types.ts)
...
Nguồn: [`packages/extensions/cordis-host-runner/src/index.ts:88`](../packages/extensions/cordis-host-runner/src/index.ts)
...
Nguồn: [`packages/credentials/credentials-local/src/index.ts:55`](../packages/credentials/credentials-local/src/index.ts)
...
Nguồn: [`packages/e2b/e2b/src/index.ts:43`](../packages/e2b/e2b/src/index.ts)
...
Nguồn: [`packages/fs/fs-local/src/index.ts:41`](../packages/fs/fs-local/src/index.ts)

<a id="deepseek-aidsh-fs-sandbox"></a>

## `@deepseek-ai/dsh-fs-sandbox`

Yêu cầu: `sandboxPolicy`
...
Phụ thuộc: [`LocalConfig`](#deepseek-aidsh-fs-local)

Nguồn: [`packages/fs/fs-sandbox/src/index.ts:49`](../packages/fs/fs-sandbox/src/index.ts)
...
Nguồn: [`packages/goal/goal/src/index.ts:116`](../packages/goal/goal/src/index.ts)

<a id="deepseek-aidsh-headless"></a>

## `@deepseek-ai/dsh-headless`

Yêu cầu: `agentDefaultModel` · `agents` · `sessions`
...
Nguồn: [`packages/bundle/headless/src/index.ts:31`](../packages/bundle/headless/src/index.ts)
...
Nguồn: [`packages/hooks/hooks-claude-code/src/index.ts:45`](../packages/hooks/hooks-claude-code/src/index.ts)
...
Nguồn: [`packages/hooks/hooks-codex/src/index.ts:44`](../packages/hooks/hooks-codex/src/index.ts)

<a id="deepseek-aidsh-host-apiproxy"></a>

## `@deepseek-ai/dsh-host-apiproxy`

Yêu cầu: `agentDefaultModel` · `agents` · `attachments` · `directoryPicker` · `llm` · `sessions` · `subagents` · `sessionQuery` · `tools` · `userInteraction` · `workspace`
...
Nguồn: [`packages/host/apiproxy/src/index.ts:41`](../packages/host/apiproxy/src/index.ts)
...
Nguồn: [`packages/host/directory-picker-browse/src/index.ts:181`](../packages/host/directory-picker-browse/src/index.ts)
...
Nguồn: [`packages/host/frontend-static/src/index.ts:28`](../packages/host/frontend-static/src/index.ts)
...
Nguồn: [`packages/host/webserver/src/index.ts:45`](../packages/host/webserver/src/index.ts)
...
Nguồn: [`packages/runtime-diagnostics/invariants/src/index.ts:15`](../packages/runtime-diagnostics/invariants/src/index.ts)
...
Nguồn: [`packages/jobs/jobs-local/src/index.ts:31`](../packages/jobs/jobs-local/src/index.ts)
...
Phụ thuộc: [`RetryPolicyConfig`](../packages/llm/llm/src/index.ts)

Nguồn: [`packages/llm/llm-deepseek/src/index.ts:62`](../packages/llm/llm-deepseek/src/index.ts)
...
Phụ thuộc: `Api`（`@earendil-works/pi-ai`）· `CacheRetention`（`@earendil-works/pi-ai`）· `Model`（`@earendil-works/pi-ai`）· `ModelThinkingLevel`（`@earendil-works/pi-ai`）· `OpenAICompletionsCompat`（`@earendil-works/pi-ai`）· [`RetryPolicyConfig`](../packages/llm/llm/src/index.ts) · `ThinkingBudgets`（`@earendil-works/pi-ai`）· `Transport`（`@earendil-works/pi-ai`）

Nguồn: [`packages/llm/llm-pi-ai/src/config.ts:172`](../packages/llm/llm-pi-ai/src/config.ts)
...
Phụ thuộc: [`ModelModality`](../packages/llm/llm/src/index.ts) · [`RetryPolicyConfig`](../packages/llm/llm/src/index.ts)

Nguồn: [`packages/test-support/llm-replay/src/index.ts:776`](../packages/test-support/llm-replay/src/index.ts)
...
Nguồn: [`packages/llm/llm-retry/src/index.ts:24`](../packages/llm/llm-retry/src/index.ts)

<a id="deepseek-aidsh-lsp-stdio"></a>

## `@deepseek-ai/dsh-lsp-stdio`

Yêu cầu: `fs` · `lsp` · `subprocess`
...
Nguồn: [`packages/lsp/lsp-stdio/src/index.ts:82`](../packages/lsp/lsp-stdio/src/index.ts)
...
Nguồn: [`packages/mcp/mcp-client/src/index.ts:98`](../packages/mcp/mcp-client/src/index.ts)

<a id="deepseek-aidsh-message-feedback"></a>

## `@deepseek-ai/dsh-message-feedback`

Yêu cầu: `storageDomain` · `sessionPersistence` · `sessions`
...
Nguồn: [`packages/feedback/message-feedback/src/index.ts:49`](../packages/feedback/message-feedback/src/index.ts)

<a id="deepseek-aidsh-permission-presets"></a>

## `@deepseek-ai/dsh-permission-presets`

Yêu cầu: `bash` · `approval` · `sessions`
...
Phụ thuộc: [`ApprovalPolicy`](subsystems/approval.md) · [`SandboxMode`](subsystems/sandbox.md)

Nguồn: [`packages/interaction/permission-presets/src/index.ts:140`](../packages/interaction/permission-presets/src/index.ts)

<a id="deepseek-aidsh-persona"></a>

## `@deepseek-ai/dsh-persona`

Yêu cầu: `systemPrompt`
...
Nguồn: [`packages/preset/persona/src/index.ts:34`](../packages/preset/persona/src/index.ts)

<a id="deepseek-aidsh-plan-mode"></a>

## `@deepseek-ai/dsh-plan-mode`

Yêu cầu: `tools` · `systemPrompt`
...
Nguồn: [`packages/plan/plan-mode/src/index.ts:70`](../packages/plan/plan-mode/src/index.ts)
...
Nguồn: [`packages/shell/pwsh-local/src/index.ts:58`](../packages/shell/pwsh-local/src/index.ts)
...
Phụ thuộc: [`LocalConfig`](#deepseek-aidsh-pwsh-local)

Nguồn: [`packages/shell/pwsh-sandbox/src/index.ts:40`](../packages/shell/pwsh-sandbox/src/index.ts)
...
Nguồn: [`packages/guard/repeat-tool-reminder/src/index.ts:28`](../packages/guard/repeat-tool-reminder/src/index.ts)
...
Nguồn: [`packages/sandbox/sandbox-local/src/index.ts:44`](../packages/sandbox/sandbox-local/src/index.ts)
...
Phụ thuộc: [`SandboxMode`](subsystems/sandbox.md)

Nguồn: [`packages/sandbox/sandbox-policy/src/index.ts:67`](../packages/sandbox/sandbox-policy/src/index.ts)
...
Phụ thuộc: `Readable`（`node:stream`）· `Writable`（`node:stream`）

Nguồn: [`packages/sdk/server/src/index.ts:29`](../packages/sdk/server/src/index.ts)
...
Nguồn: [`packages/session/session-persistence-jsonl/src/index.ts:60`](../packages/session/session-persistence-jsonl/src/index.ts)
...
Nguồn: [`packages/session/session-persistence-sqlite/src/index.ts:70`](../packages/session/session-persistence-sqlite/src/index.ts)

<a id="deepseek-aidsh-session-projection-cache"></a>

## `@deepseek-ai/dsh-session-projection-cache`

Yêu cầu: `storageDomain` · `sessionProjections` · `sessionPersistence` · `sessions`
...
Nguồn: [`packages/session/session-projection-cache/src/index.ts:42`](../packages/session/session-projection-cache/src/index.ts)
...
Phụ thuộc: [`SessionQueryConfig`](../packages/session-query/session-query/src/index.ts)

Nguồn: [`packages/session-query/session-query-sqlite/src/index.ts:89`](../packages/session-query/session-query-sqlite/src/index.ts)

<a id="deepseek-aidsh-session-reference"></a>

## `@deepseek-ai/dsh-session-reference`

Yêu cầu: `sessionQuery`
...
Nguồn: [`packages/context/session-reference/src/config.ts:11`](../packages/context/session-reference/src/config.ts)
...
Phụ thuộc: `BatchLogRecordProcessorOptions`（`@opentelemetry/sdk-logs`）· `OTLPExporterNodeConfigBase`（`@opentelemetry/otlp-exporter-base`）

Nguồn: [`packages/session/session-telemetry-otel/src/index.ts:91`](../packages/session/session-telemetry-otel/src/index.ts)
...
Nguồn: [`packages/session/session-title/src/index.ts:79`](../packages/session/session-title/src/index.ts)

<a id="deepseek-aidsh-session-title-all-prompts-llm"></a>

## `@deepseek-ai/dsh-session-title-all-prompts-llm`

Yêu cầu: `sessionTitle` · `llm` · `sessions`
...
Phụ thuộc: [`SessionTitleLlmConfig`](../packages/session/session-title-llm/src/index.ts)

Nguồn: [`packages/session/session-title-all-prompts-llm/src/index.ts:15`](../packages/session/session-title-all-prompts-llm/src/index.ts)
...
Nguồn: [`packages/session/session-title-first-prompt-llm/src/index.ts:15`](../packages/session/session-title-first-prompt-llm/src/index.ts)
...
Nguồn: [`packages/settings/settings-file/src/index.ts:21`](../packages/settings/settings-file/src/index.ts)
...
Nguồn: [`packages/shell/shell-env/src/index.ts:29`](../packages/shell/shell-env/src/index.ts)
...
Nguồn: [`packages/skill/skill/src/index.ts:279`](../packages/skill/skill/src/index.ts)
...
Nguồn: [`packages/skill/skill-filesystem/src/index.ts:49`](../packages/skill/skill-filesystem/src/index.ts)
...
Nguồn: [`packages/spill/spill-local/src/index.ts:22`](../packages/spill/spill-local/src/index.ts)
...
Nguồn: [`packages/spill/spill-policy/src/index.ts:60`](../packages/spill/spill-policy/src/index.ts)
...
Nguồn: [`packages/storage/storage-domain/src/index.ts:52`](../packages/storage/storage-domain/src/index.ts)
...
Nguồn: [`packages/storage/storage-json/src/index.ts:27`](../packages/storage/storage-json/src/index.ts)
...
Nguồn: [`packages/storage/storage-sqlite/src/index.ts:24`](../packages/storage/storage-sqlite/src/index.ts)

<a id="deepseek-aidsh-subagent-acp"></a>

## `@deepseek-ai/dsh-subagent-acp`

Yêu cầu: `subagents` · `subprocess`
...
Nguồn: [`packages/subagent/subagent-acp/src/index.ts:27`](../packages/subagent/subagent-acp/src/index.ts)
...
Nguồn: [`packages/subagent/subagent-claude-code/src/index.ts:32`](../packages/subagent/subagent-claude-code/src/index.ts)
...
Nguồn: [`packages/subagent/subagent-codex/src/index.ts:30`](../packages/subagent/subagent-codex/src/index.ts)
...
Nguồn: [`packages/subagent/subagent-dsh-sdk/src/index.ts:29`](../packages/subagent/subagent-dsh-sdk/src/index.ts)
...
Nguồn: [`packages/subagent/subagent-fork-in-process/src/index.ts:31`](../packages/subagent/subagent-fork-in-process/src/index.ts)
...
Nguồn: [`packages/subagent/subagent-spawn-in-process/src/index.ts:25`](../packages/subagent/subagent-spawn-in-process/src/index.ts)
...
Nguồn: [`packages/e2b/subprocess-e2b/src/index.ts:25`](../packages/e2b/subprocess-e2b/src/index.ts)
...
Nguồn: [`packages/core/system-prompt/src/index.ts:186`](../packages/core/system-prompt/src/index.ts)

<a id="deepseek-aidsh-terminal-bash"></a>

## `@deepseek-ai/dsh-terminal-bash`

Yêu cầu: `pty` · `sandboxPolicy` · `subprocess`
...
Nguồn: [`packages/terminal/terminal-bash/src/config.ts:6`](../packages/terminal/terminal-bash/src/config.ts)
...
Nguồn: [`packages/context/time-context/src/index.ts:27`](../packages/context/time-context/src/index.ts)
...
Nguồn: [`packages/context/tmux-context/src/index.ts:34`](../packages/context/tmux-context/src/index.ts)
...
Nguồn: [`packages/llm/token-meter/src/types.ts:12`](../packages/llm/token-meter/src/types.ts)

<a id="deepseek-aidsh-tool-bash"></a>

## `@deepseek-ai/dsh-tool-bash`

Yêu cầu: `tools` · `bash` · `systemPrompt` · `bashEnv`
...
Nguồn: [`packages/shell/tool-bash/src/index.ts:34`](../packages/shell/tool-bash/src/index.ts)
...
Nguồn: [`packages/shell/tool-bash-persistent/src/index.ts:400`](../packages/shell/tool-bash-persistent/src/index.ts)

<a id="deepseek-aidsh-tool-fs"></a>

## `@deepseek-ai/dsh-tool-fs`

Yêu cầu: `tools` · `fs` · `systemPrompt`
...
Nguồn: [`packages/fs/tool-fs/src/index.ts:25`](../packages/fs/tool-fs/src/index.ts)

<a id="deepseek-aidsh-tool-fs-search"></a>

## `@deepseek-ai/dsh-tool-fs-search`

Yêu cầu: `tools` · `systemPrompt` · `subprocess`
...
Nguồn: [`packages/fs/tool-fs-search/src/index.ts:73`](../packages/fs/tool-fs-search/src/index.ts)

<a id="deepseek-aidsh-tool-goal"></a>

## `@deepseek-ai/dsh-tool-goal`

Yêu cầu: `agents` · `goals` · `tools` · `systemPrompt`
...
Nguồn: [`packages/goal/tool-goal/src/index.ts:26`](../packages/goal/tool-goal/src/index.ts)

<a id="deepseek-aidsh-tool-jobs"></a>

## `@deepseek-ai/dsh-tool-jobs`

Yêu cầu: `tools` · `tasks` · `systemPrompt`
...
Nguồn: [`packages/jobs/tool-jobs/src/index.ts:32`](../packages/jobs/tool-jobs/src/index.ts)

<a id="deepseek-aidsh-tool-lsp"></a>

## `@deepseek-ai/dsh-tool-lsp`

Yêu cầu: `tools` · `lsp` · `systemPrompt`
...
Nguồn: [`packages/lsp/tool-lsp/src/index.ts:58`](../packages/lsp/tool-lsp/src/index.ts)
...
Nguồn: [`packages/shell/tool-pwsh/src/index.ts:52`](../packages/shell/tool-pwsh/src/index.ts)

<a id="deepseek-aidsh-tool-ralph"></a>

## `@deepseek-ai/dsh-tool-ralph`

Yêu cầu: `tools` · `workflows` · `subagents` · `systemPrompt`
...
Nguồn: [`packages/workflow/tool-ralph/src/index.ts:23`](../packages/workflow/tool-ralph/src/index.ts)

<a id="deepseek-aidsh-tool-session-query"></a>

## `@deepseek-ai/dsh-tool-session-query`

Yêu cầu: `tools` · `systemPrompt` · `sessionQuery`
...
Nguồn: [`packages/session-query/tool-session-query/src/index.ts:29`](../packages/session-query/tool-session-query/src/index.ts)

<a id="deepseek-aidsh-tool-skill"></a>

## `@deepseek-ai/dsh-tool-skill`

Yêu cầu: `agents` · `tools` · `skills`
...
Nguồn: [`packages/skill/tool-skill/src/index.ts:61`](../packages/skill/tool-skill/src/index.ts)
...
Nguồn: [`packages/fs/tool-str-replace-editor/src/index.ts:497`](../packages/fs/tool-str-replace-editor/src/index.ts)

<a id="deepseek-aidsh-tool-subagent"></a>

## `@deepseek-ai/dsh-tool-subagent`

Yêu cầu: `tools` · `subagents` · `systemPrompt`
...
Phụ thuộc: [`AgentOptions`](subsystems/core.md)

Nguồn: [`packages/subagent/tool-subagent/src/index.ts:29`](../packages/subagent/tool-subagent/src/index.ts)

<a id="deepseek-aidsh-tool-subagent-report"></a>

## `@deepseek-ai/dsh-tool-subagent-report`

Yêu cầu: `subagents` · `tools` · `systemPrompt`
...
Phụ thuộc: [`SubagentReportDelivery`](subsystems/subagent.md)

Nguồn: [`packages/subagent/tool-subagent-report/src/index.ts:27`](../packages/subagent/tool-subagent-report/src/index.ts)

<a id="deepseek-aidsh-tool-terminal"></a>

## `@deepseek-ai/dsh-tool-terminal`

Yêu cầu: `pty` · `tools` · `systemPrompt`
...
Nguồn: [`packages/terminal/tool-terminal/src/index.ts:35`](../packages/terminal/tool-terminal/src/index.ts)
...
Nguồn: [`packages/todo/tool-todo/src/index.ts:29`](../packages/todo/tool-todo/src/index.ts)

<a id="deepseek-aidsh-tool-web"></a>

## `@deepseek-ai/dsh-tool-web`

Yêu cầu: `tools` · `web` · `systemPrompt`
...
Nguồn: [`packages/web/tool-web/src/index.ts:37`](../packages/web/tool-web/src/index.ts)

<a id="deepseek-aidsh-tool-workflow"></a>

## `@deepseek-ai/dsh-tool-workflow`

Yêu cầu: `tools` · `workflows` · `systemPrompt`
...
Nguồn: [`packages/workflow/tool-workflow/src/index.ts:33`](../packages/workflow/tool-workflow/src/index.ts)
...
Nguồn: [`packages/core/tools/src/index.ts:654`](../packages/core/tools/src/index.ts)

<a id="deepseek-aidsh-typert-loader"></a>

## `@deepseek-ai/dsh-typert-loader`

Yêu cầu: `typert` · `loader`
...
Nguồn: [`packages/typert/loader/src/index.ts:47`](../packages/typert/loader/src/index.ts)
...
Nguồn: [`packages/interaction/user-approval/src/index.ts:177`](../packages/interaction/user-approval/src/index.ts)
...
Nguồn: [`packages/web/web/src/index.ts:55`](../packages/web/web/src/index.ts)
...
Nguồn: [`packages/bundle/web-app/src/index.ts:42`](../packages/bundle/web-app/src/index.ts)
...
Nguồn: [`packages/web/web-fetch-http/src/index.ts:34`](../packages/web/web-fetch-http/src/index.ts)
...
Nguồn: [`packages/web/web-search-deepseek/src/index.ts:46`](../packages/web/web-search-deepseek/src/index.ts)
...
Nguồn: [`packages/web/web-search-exa/src/index.ts:38`](../packages/web/web-search-exa/src/index.ts)
...
Nguồn: [`packages/web/web-search-perplexity/src/index.ts:32`](../packages/web/web-search-perplexity/src/index.ts)
...
Nguồn: [`packages/workflow/workflow-worker-thread/src/index.ts:32`](../packages/workflow/workflow-worker-thread/src/index.ts)

## Plugin có thể tải nhưng không có cấu hình

Các plugin này được tải qua một entry trong `cordis.yml` không chứa khối `config:`; chúng không khai báo bất kỳ interface cấu hình nào.

- `@deepseek-ai/dsh-agent`（[`packages/core/agent/src/index.ts`](../packages/core/agent/src/index.ts)）
- `@deepseek-ai/dsh-api-gateway` — Yêu cầu `typert`（[`packages/api/gateway/src/index.ts`](../packages/api/gateway/src/index.ts)）
- `@deepseek-ai/dsh-api-remotes`（[`packages/api/remotes/src/index.ts`](../packages/api/remotes/src/index.ts)）
- `@deepseek-ai/dsh-client-locale`（[`packages/client/locale/src/index.ts`](../packages/client/locale/src/index.ts)）
- `@deepseek-ai/dsh-client-modules` — Yêu cầu `webServer` · `loader`（[`packages/client/modules/src/index.ts`](../packages/client/modules/src/index.ts)）
- `@deepseek-ai/dsh-client-runtime`（[`packages/client/runtime/src/index.ts`](../packages/client/runtime/src/index.ts)）
- `@deepseek-ai/dsh-client-ui-agent-preset`（[`packages/client/ui-agent-preset/src/index.ts`](../packages/client/ui-agent-preset/src/index.ts)）
- `@deepseek-ai/dsh-client-ui-commands`（[`packages/client/ui-commands/src/index.ts`](../packages/client/ui-commands/src/index.ts)）
- `@deepseek-ai/dsh-client-ui-conversation`（[`packages/client/ui-conversation/src/index.ts`](../packages/client/ui-conversation/src/index.ts)）
- `@deepseek-ai/dsh-client-ui-cordis`（[`packages/extensions/ui-cordis/src/index.ts`](../packages/extensions/ui-cordis/src/index.ts)）
- `@deepseek-ai/dsh-client-ui-deliverables` — Yêu cầu `systemPrompt`（[`packages/client/ui-deliverables/src/index.ts`](../packages/client/ui-deliverables/src/index.ts)）
...
- `@deepseek-ai/dsh-command-compact` — Yêu cầu `commands` · `compact`（[`packages/compaction/command-compact/src/index.ts`](../packages/compaction/command-compact/src/index.ts)）
- `@deepseek-ai/dsh-command-feedback` — Yêu cầu `commands`（[`packages/feedback/command-feedback/src/index.ts`](../packages/feedback/command-feedback/src/index.ts)）
- `@deepseek-ai/dsh-command-goal` — Yêu cầu `commands` · `goals`（[`packages/goal/command-goal/src/index.ts`](../packages/goal/command-goal/src/index.ts)）
- `@deepseek-ai/dsh-commands`（[`packages/interaction/commands/src/index.ts`](../packages/interaction/commands/src/index.ts)）
- `@deepseek-ai/dsh-cordis-client-runner`（[`packages/extensions/cordis-client-runner/src/index.ts`](../packages/extensions/cordis-client-runner/src/index.ts)）
- `@deepseek-ai/dsh-fs-e2b` — Yêu cầu `e2b`（[`packages/e2b/fs-e2b/src/index.ts`](../packages/e2b/fs-e2b/src/index.ts)）
- `@deepseek-ai/dsh-fs-observation-policy`（[`packages/fs/fs-observation-policy/src/index.ts`](../packages/fs/fs-observation-policy/src/index.ts)）
- `@deepseek-ai/dsh-goal-round-driver` — Yêu cầu `agents` · `goals` · `sessions`（[`packages/goal/goal-round-driver/src/index.ts`](../packages/goal/goal-round-driver/src/index.ts)）
- `@deepseek-ai/dsh-host-directory-picker-auto` — Yêu cầu `webServer` · `loader`（[`packages/host/directory-picker-auto/src/index.ts`](../packages/host/directory-picker-auto/src/index.ts)）
- `@deepseek-ai/dsh-host-directory-picker-native`（[`packages/host/directory-picker-native/src/index.ts`](../packages/host/directory-picker-native/src/index.ts)）
- `@deepseek-ai/dsh-host-plugin-inventory` — Yêu cầu `loader`（[`packages/host/plugin-inventory/src/index.ts`](../packages/host/plugin-inventory/src/index.ts)）
- `@deepseek-ai/dsh-llm`（[`packages/llm/llm/src/index.ts`](../packages/llm/llm/src/index.ts)）
- `@deepseek-ai/dsh-lsp`（[`packages/lsp/lsp/src/index.ts`](../packages/lsp/lsp/src/index.ts)）
- `@deepseek-ai/dsh-schedule` — Yêu cầu `agents` · `sessions` · `tools` · `sessionPersistence`（[`packages/schedule/schedule/src/index.ts`](../packages/schedule/schedule/src/index.ts)）
- `@deepseek-ai/dsh-session`（[`packages/core/session/src/index.ts`](../packages/core/session/src/index.ts)）
- `@deepseek-ai/dsh-session-access` — Yêu cầu `storageDomain`（[`packages/session/session-access/src/index.ts`](../packages/session/session-access/src/index.ts)）
- `@deepseek-ai/dsh-session-checkpoint-policy` — Yêu cầu `llm` · `sessionPersistence` · `sessions` · `tools`（[`packages/session/session-checkpoint-policy/src/index.ts`](../packages/session/session-checkpoint-policy/src/index.ts)）
- `@deepseek-ai/dsh-session-log-export` — Yêu cầu `commands`（[`packages/session-query/session-log-export/src/index.ts`](../packages/session-query/session-log-export/src/index.ts)）
- `@deepseek-ai/dsh-session-projection`（[`packages/session/session-projection/src/index.ts`](../packages/session/session-projection/src/index.ts)）
- `@deepseek-ai/dsh-session-stats` — Yêu cầu `sessionProjections`（[`packages/session/session-stats/src/index.ts`](../packages/session/session-stats/src/index.ts)）
- `@deepseek-ai/dsh-skill-badge` — Yêu cầu `skills`（[`packages/skill/skill-badge/src/index.ts`](../packages/skill/skill-badge/src/index.ts)）
- `@deepseek-ai/dsh-storage`（[`packages/storage/storage/src/index.ts`](../packages/storage/storage/src/index.ts)）
- `@deepseek-ai/dsh-subagent`（[`packages/subagent/subagent/src/index.ts`](../packages/subagent/subagent/src/index.ts)）
- `@deepseek-ai/dsh-subprocess-local`（[`packages/subprocess/subprocess-local/src/index.ts`](../packages/subprocess/subprocess-local/src/index.ts)）
- `@deepseek-ai/dsh-terminal`（[`packages/terminal/terminal/src/index.ts`](../packages/terminal/terminal/src/index.ts)）
- `@deepseek-ai/dsh-tool-ask-user` — Yêu cầu `tools` · `userInteraction`（[`packages/interaction/tool-ask-user/src/index.ts`](../packages/interaction/tool-ask-user/src/index.ts)）
- `@deepseek-ai/dsh-tool-call-timeout-policy` — Yêu cầu `tools`（[`packages/guard/timeout-policy/src/index.ts`](../packages/guard/timeout-policy/src/index.ts)）
- `@deepseek-ai/dsh-tool-cordis` — Yêu cầu `tools` · `systemPrompt` · `dynamicCordisRunner` · `cordisInspect`（[`packages/extensions/tool-cordis/src/index.ts`](../packages/extensions/tool-cordis/src/index.ts)）
- `@deepseek-ai/dsh-tool-subagent-control` — Yêu cầu `tools` · `subagents`（[`packages/subagent/tool-subagent-control/src/index.ts`](../packages/subagent/tool-subagent-control/src/index.ts)）
- `@deepseek-ai/dsh-user-questions`（[`packages/interaction/user-questions/src/index.ts`](../packages/interaction/user-questions/src/index.ts)）
- `@deepseek-ai/dsh-workspace` — Yêu cầu `storageDomain` · `sessionPersistence`（[`packages/workspace/workspace/src/index.ts`](../packages/workspace/workspace/src/index.ts)）

## Package seam (không thể tải trực tiếp)

Các lớp service trừu tượng — khi triển khai nên tải package hiện thực cụ thể thay thế (xem [capability seam](../.agents/notes/implemented/architecture/2026-06-13-capability-seams.md)).

- `@deepseek-ai/dsh-attachment` — Trừu tượng hóa `AttachmentStore`（[`packages/attachment/attachment/src/index.ts`](../packages/attachment/attachment/src/index.ts)）
- `@deepseek-ai/dsh-code-runtime` — Trừu tượng hóa `CodeRuntime`（[`packages/code-runtime/code-runtime/src/index.ts`](../packages/code-runtime/code-runtime/src/index.ts)）
- `@deepseek-ai/dsh-compaction` — Trừu tượng hóa `CompactionEngine`（[`packages/compaction/compaction/src/index.ts`](../packages/compaction/compaction/src/index.ts)）
- `@deepseek-ai/dsh-credentials` — Trừu tượng hóa `Credentials`（[`packages/credentials/credentials/src/index.ts`](../packages/credentials/credentials/src/index.ts)）
- `@deepseek-ai/dsh-fs` — Trừu tượng hóa `FileSystem`（[`packages/fs/fs/src/index.ts`](../packages/fs/fs/src/index.ts)）
- `@deepseek-ai/dsh-host-directory-picker` — Trừu tượng hóa `DirectoryPicker`（[`packages/host/directory-picker/src/index.ts`](../packages/host/directory-picker/src/index.ts)）
- `@deepseek-ai/dsh-jobs` — Trừu tượng hóa `JobRegistry`（[`packages/jobs/jobs/src/index.ts`](../packages/jobs/jobs/src/index.ts)）
- `@deepseek-ai/dsh-sandbox` — Trừu tượng hóa `SandboxProvider`（[`packages/sandbox/sandbox/src/index.ts`](../packages/sandbox/sandbox/src/index.ts)）
- `@deepseek-ai/dsh-session-persistence` — Trừu tượng hóa `SessionPersistence`（[`packages/session/session-persistence/src/index.ts`](../packages/session/session-persistence/src/index.ts)）
- `@deepseek-ai/dsh-session-query` — Trừu tượng hóa `SessionQueryEngine`（[`packages/session-query/session-query/src/index.ts`](../packages/session-query/session-query/src/index.ts)）
- `@deepseek-ai/dsh-settings` — Trừu tượng hóa `Settings`（[`packages/settings/settings/src/index.ts`](../packages/settings/settings/src/index.ts)）
- `@deepseek-ai/dsh-shell` — Trừu tượng hóa `ShellExecutor`（[`packages/shell/shell/src/index.ts`](../packages/shell/shell/src/index.ts)）
- `@deepseek-ai/dsh-spill` — Trừu tượng hóa `SpillStore`（[`packages/spill/spill/src/index.ts`](../packages/spill/spill/src/index.ts)）
- `@deepseek-ai/dsh-subprocess` — Trừu tượng hóa `SubprocessRuntime`（[`packages/subprocess/subprocess/src/index.ts`](../packages/subprocess/subprocess/src/index.ts)）
- `@deepseek-ai/dsh-workflow` — Trừu tượng hóa `WorkflowEngine`（[`packages/workflow/workflow/src/index.ts`](../packages/workflow/workflow/src/index.ts)）
## Package thư viện (không có entry plugin)

Được các package khác import như thư viện; `cordis.yml` không thể tải chúng.
