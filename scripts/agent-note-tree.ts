...
export function walkAgentNoteTree(): { notes: AgentNote[]; errors: string[] } {
  ...
  for (const lifecycle of AGENT_NOTE_LIFECYCLES) {
    for (const match of globSync(`${lifecycle}/**/*.md`, { cwd: agentNoteRoot }).map(path => path.split(sep).join('/')).sort()) {
      ...
      // A Chinese (foo.zh.md) or Vietnamese (foo.vi.md) counterpart is the SAME Agent
      // Note, indexed via its English filename; the pairing gate owns its consistency.
      if (match.endsWith('.zh.md') || match.endsWith('.vi.md')) continue
      const cls = segs[1]
      const base = segs[2]
