...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/session/session-access': { kind: 'none', reason: 'The per-session access-control list is host authorization metadata; it registers nothing model-facing.' },
  ...
  'packages/identity/user-ticket': { kind: 'none', reason: 'A pure ticket-verification library; the ticket value and user id are transport credentials that never reach a model request.' },
