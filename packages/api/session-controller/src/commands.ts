...
export class SessionCommandController {
  ...
  async readWorkspaceFile(request: SessionReadWorkspaceFileRequest): Promise<SessionReadWorkspaceFileValue> {
    ...
    try {
      ...
      throw new RemoteError('gateway/internal', `workspace read unavailable for session "${request.sessionId}": ${String(error)}`, {})
    }
    const cwd = source.header.cwd
    if (cwd === undefined) {
      throw new RemoteError('gateway/bad-request', 'this session has no workspace directory to read from', {})
    }
    const target = resolve(cwd, request.path)
    const containment = relative(cwd, target)
    if (containment.startsWith('..') || isAbsolute(containment)) {
      throw new RemoteError('gateway/bad-request', `path escapes the session workspace: ${request.path}`, {})
    }
    const info = await stat(target).catch((error: unknown) => {
      throw new RemoteError('gateway/bad-request', `cannot read ${request.path}: ${String(error)}`, {})
    })
    if (!info.isFile()) throw new RemoteError('gateway/bad-request', `not a file: ${request.path}`, {})
  ...
  async uploadWorkspaceFile(request: SessionUploadWorkspaceFileRequest): Promise<SessionUploadWorkspaceFileValue> {
    ...
    try {
      ...
      throw new RemoteError('gateway/internal', `workspace upload unavailable for session "${request.sessionId}": ${String(error)}`, {})
    }
    const cwd = source.header.cwd
    if (cwd === undefined) {
      throw new RemoteError('gateway/bad-request', 'this session has no workspace directory to upload into', {})
    }
    const clean = basename(request.name).replace(/[\u0000-\u001f]/g, '').replace(/^\.+/, '')
    if (clean === '') throw new RemoteError('gateway/bad-request', `unusable file name: ${request.name}`, {})
