...
| `presetWorkspaceRoot` | 未设置 | 未指定 Workspace 与 cwd 的会话，其 cwd 取该目录下的 `<root>/<presetId>` 子目录（`~` 会展开）；未设置时沿用进程工作目录。子目录随会话创建，并在 `agent-preset/authored` 时提前建立，便于先行放置文件。每个预设目录还会得到一个以预设显示名命名的 Workspace 实体（`agent-preset/renamed` 时同步改名）：落在该目录的会话归属其下，首次 `session/list` 前的一次性惰性对账会接管既有预设目录及其已存会话，使侧边栏按预设分组而非 Ungrouped |
| `ticketSecret` | 未设置 | 校验用户票据（`mtil-ticket` cookie）的共享 HMAC-SHA256 密钥。设置后，`session/list`/`session/search` 按每个会话的 `session_access` 允许用户记录过滤（记录缺失或为空即不受限，对所有人可见；匿名调用者只看到不受限的会话），新会话会写入已识别的创建者，`session/setAccess` 用于替换记录。用户 `*` 的票据是管理通配符：可看到全部会话及其记录，且是 `session/setAccess` 唯一接受的调用者。会话相关的读取与命令（page、follow、prompt、rename、fork、selectModel、attachment、updateQueue、cancel）在调用者未被记录列出时，返回与未知 id 相同的 `session-not-found`。未设置时所有调用者均为匿名 |
