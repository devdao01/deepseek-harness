...
| `ticketSecret` | unset | Shared HMAC-SHA256 secret user tickets (the `mtil-ticket` cookie) are verified with. Set, `session/list`/`session/search` filter by each session's `session_access` allowed-users record (absent/empty record = unrestricted, visible to everyone; anonymous callers see unrestricted sessions only), an identified creator is written into a new session's record, and `session/setAccess` replaces a record. Unset, every caller is anonymous |
