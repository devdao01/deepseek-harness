/**
 * Preset-conventional workspace paths. Provisioning an agent preset also
 * provisions a per-preset workspace under a conventional root — preset
 * `accounting` maps to `<root>/accounting` — and the same convention lets the
 * controller find a preset's workspace again from the preset id alone, since
 * the 0.1.2 preset roster stores no workspace link. This module owns the pure
 * path logic: resolving the configured root (with its load-time validation) and
 * mapping a preset id to its directory. All filesystem and registry effects
 * live in the controller; nothing here touches disk.
 * @module @deepseek-ai/dsh-api-preset-workspace-controller/preset-workspace
 */
 ...
 * is the explicit defaulting step the controller runs at construction, not a
 * hidden fallback: an absent value becomes `<home>/workspace`; a `~/`-prefixed
 * value expands against the home directory; any other value must already be
