...
`trustedHosts` 与 `auth` 都在代码中从可选的 `webRuntime` 服务默认（通过 `ctx.get` 读取，因此本插件在没有它时仍可组合）：当部署两者都未配置时，`trustedHosts` 继承 `webRuntime.trustedHosts`（其 LAN 字面量加 `--trusted-host` 额外项），`auth` 从 `webRuntime.apiToken` 派生一个 `web` token——授予 `agentPreset.read`/`copy`/`openDocument`/`remove` 创作钉住方法。因此在 web 运行时（`dsh web`）旁挂载本插件的组合默认即为强制认证，而没有 webRuntime 的组合保持只限回环、仅栅栏，与此前完全一致。显式的 `trustedHosts`/`auth` 配置始终整体胜出，且派生的默认 `unpinned` 集合仅对已认证客户端生效（无 token → 该表面不可达，因此把它作为代码默认发布不扩大任何权限）；settings、credentials 与原生宿主钉住方法一如既往保持关闭。
