...
async function serve(downlinks: WebSocketDownlinks): Promise<{
  ...
  server.on('upgrade', (request, socket, head) => {
    ...
    if (pathname === MUX_EVENTS_PATH) downlinks.handleMux(request, socket, head, { kind: 'token' })
    else if (pathname === HOST_EVENTS_PATH) downlinks.handleHost(request, socket, head, { kind: 'token' })
    else socket.destroy()
  })
