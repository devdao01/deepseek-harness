...
Application discovery, launch requests, and icon URLs preserve the absolute `globalThis.__DSH_APP_BASE__` set before browser boot, including bases without a trailing slash. Without it, URLs use the page origin or `http://dsh.internal` when the origin is unavailable or `null`.
