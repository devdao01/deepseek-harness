...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'sidebar.header.actions',
    kind: 'list',
    scope: 'root',
    summary: 'Optional action buttons between the expanded brand and the collapse toggle (theme/language switches, deployment shortcuts).',
    doc: 'Optional action buttons between the expanded brand and the collapse\ntoggle (theme/language switches, deployment shortcuts). Declared by\nthis package\'s `sidebar` entry; empty deployments render nothing.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'sidebar.header.actions\', () => ctx.slots.register(\n      { name: \'sidebar.header.actions\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
