...
工作区文件下载是第二个宿主侧下载面，不是 RPC：`GET /api/workspace.file?sessionId=…&path=…` 把该会话工作区目录中的一个常规文件流式返回为 `application/octet-stream` 附件（`content-length` 取自 stat，`content-disposition` 以文件的 basename 命名并附带 RFC 5987 ext-value，使非 ASCII 名称得以保留）。`path` 为绝对路径，或相对于工作区。约束（containment）是安全核心：会话自身的 cwd（其实时会话或持久化 header，在不创建或恢复 Agent 的前提下解析）为根，根与被请求文件都会经 `realpath` 规范化，使符号链接无法逃逸——规范化后的文件必须等于根，或位于 `canonicalRoot + sep` 之下。可达性即部署的信任边界（该 API 不带鉴权）；可达的客户端已能通过提示 Agent 泄露任意工作区文件，因此一次受约束的工作区读取不扩大任何权限。未知会话、缺失文件或非常规文件（目录、fifo……）应答 404；解析到工作区之外的文件应答 403。受约束的常规文件在其为可执行工件时同样应答 403——其小写扩展名在一个固定集合中（`exe`、`dll`、`so`、`dylib`、`sh`、`bat`、`ps1`、`py`、`rb`、`jar`、`bin`、`run`、`elf`…… 属安全不变量，绝非部署配置），或其 stat mode 带有 POSIX 执行位（`mode & 0o111`；在 Windows 上由扩展名列表承载全部策略）——使该通道无法成为把可运行二进制与脚本从宿主拉走的便捷途径；前端镜像同一扩展名列表以隐藏失效的下载链接。`HEAD` 返回相同的状态与响应头但无 body。端点由传输层挂载，`ApiProxy.downloads.workspaceFile` 实现它。
