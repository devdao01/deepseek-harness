...
- `ctx.agentPresets.setDisplay(id, { name?, description?, disabled? }): Promise<void>` Set a locally authored preset's display name, description, and/or `disabled` state in its `preset.yml`, keeping its `order` and stamped workspace path. This is the edit `copy` cannot make: a copy keeps the source's description with no later way to change it, and its name is the id fallback until one is chosen. A field present in `updates` is applied — a non-empty display string sets it, an empty or whitespace one clears it, `disabled: true` turns the preset off and `disabled: false` re-enables it — while an absent key keeps the current value; clearing every display field of an enabled preset removes the metadata file so the preset publishes nothing. Only a `user` preset under the writable root may be edited, the same ownership guard as `remove`. The underlying `writePresetDisplay(roots, preset, updates)` is exported for callers holding a resolved preset.

`AgentPreset` carries `id` (the directory name), `trust` (`system` or `user`, from the root it was found under), `path` (the absolute composition file), `workspacePath` (the absolute default-workspace directory a preset stored at authoring time, absent for shipped presets and pre-stamp copies), `disabled` (`true` only when the preset was switched off on purpose — it stays on the roster but every mounting path refuses it), and — only when the preset cannot compose a session — `broken` (one human-readable reason, shown verbatim on roster surfaces).
...
It carries display text plus two non-display fields. `workspacePath` is the preset's absolute default-workspace directory, stamped by `setWorkspacePath` after that workspace is provisioned (a relative or malformed value reads as absent, the same non-fatal rule as the display fields). `disabled: true` turns the preset off: it stays on the roster so a surface can present and re-enable it, but every mounting path refuses it exactly as it refuses a broken one — only the literal boolean `true` counts, so a hand-edited `disabled: no` or a stray string reads as enabled. `id` is the directory name and `trust` comes from the root the preset was discovered under, so neither is writable here — otherwise a locally authored preset could name itself into the shipped set. It is a separate file because the composition is a top-level list of plugin rows: YAML cannot carry sibling keys beside it, and a fake metadata row would hand the Loader something to load.
