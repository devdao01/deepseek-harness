...
**拒绝可执行工件。** 在约束与常规文件检查通过后，`isExecutableArtifact(basename, stat.mode)` 会在文件为可执行工件时以 403 拒绝：其小写扩展名（basename 最后一个点之后的片段；无点或尾随点即无扩展名）在固定的 `EXECUTABLE_EXTENSIONS` 集合中，或其 stat mode 带有任意 POSIX 执行位（`mode & 0o111`）。该集合与 mode 检查是固定的安全不变量，而非部署配置：该通道用于把数据文件交给浏览器，而一个重新放行可执行文件的开关会悄悄把它变成把可运行二进制与脚本外泄、再打包的便捷途径。在 Windows 上 mode 从不设置执行位，因此那里由扩展名列表承载全部策略。前端镜像同一扩展名列表以隐藏失效的下载链接；该服务端拒绝是执行点。
...
SPA 可以把工具卡片的 location 直接链接到 `GET /api/workspace.file`，浏览器即以其真实名称下载文件——除非它是可执行工件，此时应答 403。该通道非模型可见且不发出会话事件，因此不涉及快照 fixture，也不涉及 `SESSION_FORMAT_VERSION` 提升。它的代价是载体必须与 RPC 边界保持一致地维护的第二个无信封路由（两个 GET 下载路由都位于仅 POST 的守卫之上），以及对一个已 `realpath` 的越界文件在 403 之前会先做一次 `stat`——这可以接受，因为 `realpath` 已经触碰过该路径，且在证明约束之前只读取元数据、绝不读取内容。可执行拒绝是一个粗粒度的尽力而为过滤：它拦截常见的可运行扩展名与执行位，而非每一种可能的解释器输入（数据文件仍可通过显式命名被运行），因为它的职责是阻止该通道成为*便捷的*二进制／脚本拉取途径，而非证明不可执行性。该固定列表出于必要在前端被复制；两份副本必须一起变更，而本 note 与 `EXECUTABLE_EXTENSIONS` 的 JSDoc 是共享的事实来源。
